...
# Replace the -1 ie missing values in with NAs 
...
colsum <- colSums(train=="-1",na.rm = TRUE) 
colsum[colsum>0]
```
...
# Replace the -1 ie missing values in with NAs 
# Find the index of the columns names that contain cat
indx <- grepl('cat', colnames(test))
# First convert the categorical columnds to NA
test <- replace.value( test, colnames(test)[indx], from=-1, to=NA, verbose = FALSE)

# Next convert the numerical columns to NA
test <- replace.value( test, colnames(test)[-indx], from=-1, to=NA, verbose = FALSE)

# Sanity check that we have cleaned up all -1's, the result should be empty
colsum <- colSums(test=="-1",na.rm=TRUE) 
colsum[colsum>0]
```
